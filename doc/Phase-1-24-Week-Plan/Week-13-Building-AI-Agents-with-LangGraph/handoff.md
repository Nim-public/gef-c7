# Handoff — Week 13 Building AI Agents with LangGraph: Deep Expansion Phase
...
| `Week-13-Building-AI-Agents-with-LangGraph.md` | generated overview — do not modify |
| `01-langgraph-foundations.md` | topic deep-dive |
| `02-project-story-generator.md` | topic deep-dive |
| `03-project-support-ticket-router.md` | topic deep-dive |
| `04-team-agents-codegen-loop.md` | topic deep-dive |
| `05-capstone-task-phidata-agent.md` | topic deep-dive |
| `06-checkpointing-human-in-loop.md` | topic deep-dive |
...
#### `01-langgraph-foundations.md` → subfolder `01-langgraph-foundations/`

Deep-dive files to create (suggested titles — refine as you write):

1. State design — TypedDict, Pydantic, reducers
2. Nodes and edges — normal and conditional wiring
3. Cycles and bounds — retry counters, exit conditions
4. Invoke/stream/inspect — execution path reading

#### `02-project-story-generator.md` → subfolder `02-project-story-generator/`

Deep-dive files to create (suggested titles — refine as you write):

1. Story state — chapters, world, options
2. The WAIT pattern — human choice pausing
3. Choice application — world JSON updates with fallback
4. Transferable pattern — interactive flows generally

#### `03-project-support-ticket-router.md` → subfolder `03-project-support-ticket-router/`

Deep-dive files to create (suggested titles — refine as you write):

1. Classification node — structured outputs, reasoning
2. Escalation edges — urgency gating first
3. KB/data nodes — W9/W6 capstone integration
4. Three-way router comparison — rules/handoffs/graph

#### `04-team-agents-codegen-loop.md` → subfolder `04-team-agents-codegen-loop/`

Deep-dive files to create (suggested titles — refine as you write):

1. Self-repair graph — plan/write/test/debug cycle
2. Sandbox discipline — subprocess/container hardening
3. Supervisor topology — routing workers
4. Team vs single — measured A/B

#### `05-capstone-task-phidata-agent.md` → subfolder `05-capstone-task-phidata-agent/`

Deep-dive files to create (suggested titles — refine as you write):

1. Agno assembly — toolkits+knowledge in one agent
2. 15 data-intensive cases — design and gold answers
3. Numeric grounding — verification nodes
4. Comparison vs W11 — same-case tables

#### `06-checkpointing-human-in-loop.md` → subfolder `06-checkpointing-human-in-loop/`

Deep-dive files to create (suggested titles — refine as you write):

1. Checkpointers — threads, durability, storage choice
2. interrupt_before — approval design and resumes
3. State editing — human corrections mid-run
4. Time travel — replay and fork debugging
...
doc/Phase-1-24-Week-Plan/Week-13-Building-AI-Agents-with-LangGraph/handoff.md and follow it completely.
