# Handoff — Week 15 Production Grade Agent Reliability Performance Optimization: Deep Expansion Phase
...
| `Week-15-Production-Grade-Agent-Reliability-Performance-Optimization.md` | generated overview — do not modify |
| `01-reliability-limits-retries-tests.md` | topic deep-dive |
| `02-tracing-guardrails-langsmith.md` | topic deep-dive |
| `03-inference-optimization.md` | topic deep-dive |
| `04-prompt-caching-and-routing.md` | topic deep-dive |
| `05-practice-production-hardening.md` | topic deep-dive |
...
#### `01-reliability-limits-retries-tests.md` → subfolder `01-reliability-limits-retries-tests/`

Deep-dive files to create (suggested titles — refine as you write):

1. RunBudget — turns/tokens/time/spend aborts
2. Retry policies — tenacity backoff, budgets, circuit breakers
3. User contracts — exception→message handler maps
4. Test pyramid — stubbed unit, contract, integration, soak

#### `02-tracing-guardrails-langsmith.md` → subfolder `02-tracing-guardrails-langsmith/`

Deep-dive files to create (suggested titles — refine as you write):

1. LangSmith setup — automatic tracing, projects
2. Datasets & evaluations — hosted regression runs
3. Platform guardrails — moderation/PII layering
4. Trace hygiene — PII scrubbing, retention, sampling

#### `03-inference-optimization.md` → subfolder `03-inference-optimization/`

Deep-dive files to create (suggested titles — refine as you write):

1. KV cache — prefill/decode memory math
2. Continuous batching — throughput mechanics
3. vLLM/SGLang serving — knobs and benchmarks
4. Serving quantization — AWQ/FP8 trade-offs

#### `04-prompt-caching-and-routing.md` → subfolder `04-prompt-caching-and-routing/`

Deep-dive files to create (suggested titles — refine as you write):

1. Prefix structuring — the stable/variable order rule
2. Cache verification — cached_tokens in billing
3. Model routing — rules→classifier→RouteLLM
4. Threshold calibration — misroute costs both ways

#### `05-practice-production-hardening.md` → subfolder `05-practice-production-hardening/`

Deep-dive files to create (suggested titles — refine as you write):

1. Baseline measurement — W14-06 numbers
2. Reliability layer — budgets/retries/handlers live
3. Optimization ledger — attributed improvements
4. Before/after table — p95, $/task, quality
...
doc/Phase-1-24-Week-Plan/Week-15-Production-Grade-Agent-Reliability-Performance-Optimization/handoff.md and follow it completely.
